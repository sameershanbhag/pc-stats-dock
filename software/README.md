...
instead of the numbers, in one of three looks (admin page › Idle face › Look): **Cute**, big soft glowing
robot eyes that squash as they move and arch into a smile when touched (the default); **Glass**, realistic glass
spheres with a fibrous luminous iris; **Classic**, the flat drawn shapes. The first two are drawn per pixel on the
GPU (a WebGL 2 fragment shader) and move on a small physics model: spring saccades with micro-movements,
asymmetric blinks, breathing. `?face&style=glass` previews a look in a browser. They wander and blink, watch where your mouse is on the big screen, narrow when the CPU
